...
// Game rooms storage
const rooms = new Map();
// Store gameOver timeouts to clear them if game restarts
const gameOverTimeouts = new Map();
...
io.on('connection', (socket) => {
  ...
  socket.on('openChamber', (targetPlayerId, chamberIndex, callback) => {
    ...
    if (gameState.foundGold === gameState.totalGold) {
      ...
      // Don't set phase yet - will set after chamber reveal shows
    } else if (gameState.foundTraps === gameState.totalTraps) {
      // All traps found - Hexen win
      gameOver = true;
      winner = 'hexen';
      ...
      if (gameState.round >= 3) {
        // Game ends after 4 rounds - Hexen win if not all gold found
        gameOver = true;
        winner = 'hexen';
    ...
    if (gameOver) {
      // Clear any existing gameOver timeout for this room
      if (gameOverTimeouts.has(socket.roomCode)) {
        clearTimeout(gameOverTimeouts.get(socket.roomCode));
      }
      
      // Delay gameOver slightly so chamber reveal shows first
      const timeoutId = setTimeout(() => {
        ...
        gameOverTimeouts.delete(socket.roomCode);
      }, 3500); // Wait for chamber reveal modal (3 seconds) + buffer
      
      // Store the timeout reference
      gameOverTimeouts.set(socket.roomCode, timeoutId);
  ...
  // Restart game (rematch in same room)
  socket.on('restartGame', (callback) => {
    ...
    console.log('Restarting game in room:', socket.roomCode);
    
    // Clear any pending gameOver timeout from previous game
    if (gameOverTimeouts.has(socket.roomCode)) {
      clearTimeout(gameOverTimeouts.get(socket.roomCode));
      gameOverTimeouts.delete(socket.roomCode);
      console.log('Cleared pending gameOver timeout');
    }
    
    // Reset game - same as initializeGame
    initializeGame(room);
    
    // Send new role and chamber information to each player
    room.players.forEach(player => {
      io.to(player.id).emit('roleAssigned', {
        role: player.role,
        chambers: player.chambers
