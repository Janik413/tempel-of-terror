...
function Game({ room, socket, playerRole: initialRole, playerChambers: initialChambers, onLeave }) {
  ...
  const [showChamberReveal, setShowChamberReveal] = useState(false);
  const [chamberRevealData, setChamberRevealData] = useState(null);
  const [playerRole, setPlayerRole] = useState(initialRole);
  const [playerChambers, setPlayerChambers] = useState(initialChambers);
  ...
  useEffect(() => {
    ...
    // Listen for role assignment (happens on game start AND restart)
    socket.on('roleAssigned', (data) => {
      console.log('=== ROLE ASSIGNED ===');
      console.log('Role:', data.role);
      console.log('Chambers:', data.chambers);
      console.log('Chamber count:', data.chambers?.length);
      
      setPlayerRole(data.role);
      setPlayerChambers(data.chambers || []);
      
      // Show role modal when we have the role and chambers
      if (data.chambers && data.chambers.length > 0) {
        console.log('Showing role modal with chambers');
        setShowRoleModal(true);
      } else {
        console.warn('No chambers received in roleAssigned!');
      }
    });
    ...
    socket.on('chamberRevealed', (data) => {
      ...
      // Show chamber reveal modal instead of message banner
      setChamberRevealData({
        playerName: data.playerName,
        chamberType: data.chamberType
      });
      setShowChamberReveal(true);
    ...
    // Game over event - update players with their roles AND set phase
    socket.on('gameOver', (data) => {
      console.log('Game.js received gameOver event:', data);
      
      // Set phase to gameOver to show the GameOver screen
      setGameState(prev => ({
        ...prev,
        phase: 'gameOver',
        winner: data.winner,
    ...
    // Game restarted event - reset everything for new game
    socket.on('gameRestarted', (data) => {
      ...
      // Clear modals EXCEPT role modal (roleAssigned will handle that)
      setShowChamberReveal(false);
      setChamberRevealData(null);
      setShowRoundTransition(false);
      setRoundTransitionData(null);
      
      // DON'T touch showRoleModal or playerChambers here
      // roleAssigned event will set those
  ...
  return (
    <div className="game-screen">
      ...
      {/* Chamber Reveal Modal */}
      {showChamberReveal && chamberRevealData && (
        <ChamberReveal
          playerName={chamberRevealData.playerName}
          chamberType={chamberRevealData.chamberType}
          onComplete={() => setShowChamberReveal(false)}
        />
      )}
        ...
        <div className="modal-overlay" onClick={() => setShowRoleModal(false)}>
          <div className="modal role-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className={`role-badge ${playerRole}`}>
              {playerRole === 'adventurer' ? '⚔️ Adventurer' : '🧙‍♀️ Hexe'}
            </div>
            <div className="role-description">
              {playerRole === 'adventurer' ? (
                <>
                  ...
                  <p>Avoid the <strong>🔥 Fire Traps</strong> - they help Hexen win!</p>
                </>
              ) : (
                <>
                  <p>You are a <strong>Hexe</strong>!</p>
